...
La construction des arbres de décision à partir de données n'est pas une discipline nouvelle, la première apparition remonte à 1963, lorsque les deux statisticiens, \textsc{Morgan} et \textsc{Sonquist}, avaient utilisés des arbres de régression dans un processus de prédiction \textsc{AID} (Automatic Interaction Détection), ce qui a ouvert la voie a un enchainement d'algorithme, dont THAID, CHAID, CART, ID3, C4.5 et C5.

ce qui avait ouvert la voie à un ensemble de méthodes, d'abord en 1973 avec \textsc{THAID} (\textsc{Morgan} et \textsc{Messenger}), ensuite en 1980, avec \emph{CHAID} (\textsc{Kass}). \\
Néomoins, cette discipline n’a pris de l’ampleur qu’à partir de 1984, avec l’apparition de la méthode \emph{CART} (Classification and Regression Tree) de \textsc{Breiman} et al. Ensuite, et grâce à \textsc{Ross Quinlan}, qui est considéré comme un des piliers de la discipline, des méthodes de classifications ont été misent au point, a commencé par \emph{3}, en 1986, ensuite, en 1993, l’amélioration de cette dernière donna naissance à une méthode considérée comme référence incontournable des arbres de décision, à savoir la méthode \emph{C4.5}. Cet algorithme à encore évolué en \textsc{C5.0} (Sous Unix) et \textsc{See5} (Sous Windows), mais en étant cette fois ci implémentée dans un logiciel commercial\cite{chen1996data}.
\newpage
\begin{figure}[!h]
\begin{center}
	\includegraphics[scale=0.35]{Images/Chapitre02/Historique.png}
	\caption{Algorithmes d'arbres de décision par ordre chronologique}
\end{center}
\end{figure}
...
&Adapté au grand volume de données & Non adapté au grand nombre de modalité d'attribut, car :\newline ça fera augmenter le nombre de ligne de la table de contingence & Non adapté au grand nombre de catégorie de classe, car :\newline ça fera augmenter le nombre de colonne de la table de contingence\\
 \hline
\multirow{2}{*}{\rotatebox[origin=c]{90}{ CART \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ }}&\cellcolor{gray!25} Temps \checkmark  & \cellcolor{gray!25}Complexité $\times$& \cellcolor{gray!25}Complexité\checkmark\\
&Indifférent à la variation de taille, car : \newline Sa formule est légère & Non adapté au grand nombre de modalité d'attribut, Car :\newline Son critère divise par valeur d'attribut et non par attribut& Adapté aux classes à catégorie binaire\\
  \hline
\multirow{2}{*}{\rotatebox[origin=c]{90}{ ID3 \ \ \ \ \ \ \ \ \ \ \ \ \ \ }}&\cellcolor{gray!25} Temps $\times$ & \cellcolor{gray!25}Complexité $\times$& \\
&Non adapté au très grand volume, à cause :\newline Des logarithmes utilisés dans le calcule du gain & Non adapté au nombre réduit de modalité, car :\newline il utilise le Gain d'information) & \\
  \hline
\multirow{2}{*}{\rotatebox[origin=c]{90}{ C4.5 \ \ \ \ \ \ \ \ \ \ \ \ \ \ }}&\cellcolor{gray!25} Temps $\times$  & \cellcolor{gray!25}Complexité \checkmark & \\
& Non adapté au très grand volume, du au calcul : \begin{itemize}
\item Gain
\item InfoSplit
\end{itemize}  & Adapté au nombre élevé de modalité (grâce du Rapport de gain) & \\
  \hline
\end{tabular}
